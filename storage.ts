...
const GLOBAL_KEYS = {
  ...
  ADMIN_PROFILE: 'madarsa_admin_profile',
...
export const storage = {
  ...
  saveAdminAuth: (creds: { username: string; password: string }) => {
  ...
  getAdminProfile: (): any => {
    const profile = localStorage.getItem(GLOBAL_KEYS.ADMIN_PROFILE);
    return profile ? JSON.parse(profile) : null;
  },

  saveAdminProfile: (profile: any) => {
    localStorage.setItem(GLOBAL_KEYS.ADMIN_PROFILE, JSON.stringify(profile));
    syncService.push('GLOBAL', 'admin_profile', profile);
  },
  ...
  syncGlobalsFromCloud: async () => {
    ...
    const adminProfile = await syncService.pull('GLOBAL', 'admin_profile');
    if (adminProfile) localStorage.setItem(GLOBAL_KEYS.ADMIN_PROFILE, JSON.stringify(adminProfile));
  },
